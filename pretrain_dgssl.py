...
from dgssl.model import GNN
from dgssl.util import MaskAtom
from dgssl.loader import MoleculeDataset
from dgssl.dataloader import DataLoaderMasking
...
def train(args, model_list, loader, optimizer_list, device):
    ...
    for step, batch in enumerate(tqdm(loader, desc="Iteration")):
        ...
        loss = criterion(pred_node.double(), batch.mask_node_label[:,2])

        acc_node = compute_accuracy(pred_node, batch.mask_node_label[:,2])
...
def main():
    ...
    parser.add_argument('--output_model_file', type=str, default = 'models/dc_e1_15', help='filename to output the model')
    ...
    linear_pred_atoms = torch.nn.Linear(args.emb_dim, 137).to(device)
